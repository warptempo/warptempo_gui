...
The plate is a direct ARGB32 coverage renderer, one synchronous full render per user-driven pan/zoom frame. The ALIASED parts that remain: per-column raw min/max interiors on the authoring lattice `g(k0+c)` (bit-exact pan invariance in both views), the 13-rung peaks pyramid (.peaks v7), and the never-fade >=1px floor (now plain integer geometry). The ANTIALIASED parts that were deleted:
