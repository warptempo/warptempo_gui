...
- **The device config's first-run template**: `GuiPlatform::device_config_defaults()`,
  ...
  both-sides member. The SEVENTEEN keys it stamps are per-DEVICE preferences
  ...
  (the waveform cap, the retired `kWaveformMaxHeightPx`'s value), the five
  waveform gain defaults (`kDefaultWaveformGainParams`), the six expander
  defaults (`kDefaultWaveformExpanderParams`, ratio 1.00: off), `kDefaultProjectsRepo` and a
