...
- **The device config's first-run template**: `GuiPlatform::device_config_defaults()`,
  ...
  both-sides member. The SEVEN keys it stamps are per-DEVICE preferences
  ...
  removals at a folder the user never named — and `fg_color=#1b9e84`
  (`kWaveformCoreInkDefault`, the lit plate's core ink for its tuning
  phase, architect 2026-09-25; settings.md). (A key the template no longer
...
- **Android stubs** (each named at its site with its Wayland twin), THREE
  ...
  no seam member of its own — and `set_title_dirty` is **DELETED FROM THE SEAM
  ON BOTH BACKENDS** the same day (architect: *"it should only show up in the
  bottom. Otherwise it becomes a duplicate signal, and we avoid those in this
  project"*), taking the laptop's titlebar asterisk with it. The title stub
  that remains is `set_project_title` alone, and the title it no-ops is the
  project name plus `" - warptempo_gui"` and nothing else. **THE CLIPBOARD LEFT THIS LIST** the evening it was
...
- **`publish_media_state`** — the push UP, from TWO owners since 2026-09-17,
  ...
  SESSION STANDS spelled as a batch cell's basename is (`5_+2` — the live
...
unrounded 748·3.08 = 2303.8 would claim 308, which is why the walk is the
measure). It was 816 and 282 from the evening of 2026-09-22, when the two were
restored (23 members in 5 groups; 2301 of 2304 at 282). It was
...
5 groups; 748*3.08 = 2303.84 of 2304 unrounded, the painted walk's own ceiling
being 307). It was 714
