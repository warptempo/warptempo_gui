...
- **The device config's first-run template**: `GuiPlatform::device_config_defaults()`,
  ...
  both-sides member. The THIRTEEN keys it stamps are per-DEVICE preferences
  ...
  (the waveform cap, the retired `kWaveformMaxHeightPx`'s value), the seven
  waveform gain defaults (`kDefaultWaveformGainParams`), `kDefaultProjectsRepo` and a
