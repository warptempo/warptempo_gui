...
**Trim.** `b` and `e` set that bound at the playhead and autoset the other bound 5 seconds away, but only when that other bound is unset (placing a bound across its partner clears the partner, so the autoset re-establishes it). Re-pressing with the playhead on a bound walks the other bound outward another 5 seconds. `Shift+B` and `Shift+E` pull the other bound inward 5 seconds instead (clamped just short of the opposite bound). `Ctrl+B` and `Ctrl+E` clear a bound unconditionally. `u` unsets the active tab's trim bounds, and the unset is undoable. `Ctrl+Shift` with a left-drag on either trim boundary stem moves the trim begin and end together as a fixed-width pair, preserving the gap between them, clamped so neither bound leaves the clip; the gesture selects both bounds. It requires both bounds to already be set and is otherwise a no-op.
