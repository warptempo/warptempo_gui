...
Each CLI takes just the source audio path (`warptempo_engine` also accepts `--no-limiter`); everything else — output format, title, trim, limiter — comes from the project's `.settings` sidecar, and outputs are title-named beside the source at exactly the paths the GUI would write for the same project.

`warptempo_parser` (`-DWARPTEMPO_BUILD_PARSER=ON`) is parser-only. It reads a project's `.warpmarkers`, `.phaseresetmarkers`, and `.settings` beside the source audio (all three required; the GUI creates them on source load) and writes the map artifacts for the project's `output_format` — the `<title>.warpframemap` plus `<title>.phaseresetframemap` pair for `warptempo_maps`, `<title>.warpframemap` alone for `generic_map`, `<title>.miditempomap` for `midi_map` — without the engine or the GUI. `output_format=wav` is refused: the engine renders wav.
...
`warptempo_engine` (`-DWARPTEMPO_BUILD_ENGINE=ON`) is engine-only and reads no authoring files. It consumes the prebuilt map pair `warptempo_parser` writes for a default-titled project — the sibling `<source-stem>-rendered.warpframemap`, plus `<source-stem>-rendered.phaseresetframemap` when present — and the source wav, runs the PGHI engine directly, and writes `<source-stem>-rendered.wav` beside the source (`limiter=false;<source-stem>-rendered.wav` with `--no-limiter`). A custom-titled map pair is reachable by renaming the files to that default-title shape. This is the entry point for driving the engine from maps produced by any source, not just this project's authoring files. Phase-reset anticipation is an authoring decision the engine never sees: the parser-side derivation applies it in target/output frames after temporal warping, then inverse-maps the anticipated target frame through the engine map and corrects by `N/2` into the engine's origin-centered source-query domain before the `.phaseresetframemap` is written, so this tool consumes the list as-is; marker files, GUI display positions, and render-view sidecars remain at the authored onset positions.
