...
**Marker focus and selection.** `Tab` and `Shift+Tab` (the latter also reachable as the ISO left-tab key some layouts send) cycle marker focus forward and backward, recentering the current zoom on the focused marker; the focus walk skips disabled markers and operates on whichever collection the W/P toggle has active. `j` jumps the focused marker to the playhead.
