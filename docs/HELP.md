...
`warptempo_parser` (`-DWARPTEMPO_BUILD_PARSER=ON`) is parser-only. It reads a project's `.warpmarkers`, `.phaseresetmarkers`, and `.settings` beside the source audio and writes the portable map artifacts — the `.warpframemap` plus `.phaseresetframemap` pair for `warptempo_maps`, the `.warpframemap` alone for `generic_map`, the `.miditempomap` for `midi_map` — without the engine or the GUI.
