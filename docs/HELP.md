...
Phase reset markers carry two pieces of state: a position and a disabled flag. They have no tempo, no labels, no scale, no mode. The engine concern they parameterize is *where to reset* — re-grounding the synthesis phase at the transient that opens the segment. The small lead-in the engine applies before each reset is a fixed two synthesis hops, applied uniformly to every reset rather than being a per-marker setting. A reset so close to the start of the output that the lead-in would land at or before output sample zero is legal but inert: the first frame seeds synthesis phase from analysis phase anyway, and the engine silently drops the redundant reset at dispatch. A marker at 00:00.000 itself parses, loads, and saves like any other.

## Output formats and the limiter

The `output_format` setting is one of `wav` (the default, finished audio), `warptempo_maps` (writes the warp frame map and phase reset frame map pair — together exactly the engine's input, for driving an external engine implementation; phase-reset anticipation is already baked into the pair by the parser-side derivation, so a consumer reads it as-is), `generic_map` (writes the warp frame map alone, for generic external stretch consumers), or `midi_map` (writes the MIDI tempo map for DAW hosts); the map formats write maps rather than audio, the limiter does not apply to them, and they refuse to render while a trim bound is set — trim is a wav-only render window, and the map artifacts are always the full maps.
