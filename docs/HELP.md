...
**Trim.** `b` and `e` set that bound at the playhead and autoset the other bound 5 seconds away (end after begin, begin before end), clamped to the clip bounds; pressing the same key again with the playhead on that bound walks the other bound out another 5 seconds. `Ctrl+B` and `Ctrl+E` set a single bound without autosetting the other (re-press toggles it off; placing it past the opposite bound disables that bound). `Shift+B` and `Shift+E` clear the trim begin and trim end unconditionally. `u` unsets the active tab's trim bounds, and the unset is undoable. `Ctrl+Shift` with a left-drag on either trim boundary stem moves the trim begin and end together as a fixed-width pair, preserving the gap between them, clamped so neither bound leaves the clip; the gesture selects both bounds. It requires both bounds to already be set and is otherwise a no-op.
