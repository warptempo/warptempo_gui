...
Each CLI takes just the source audio path (`warptempo_engine` also accepts `--no-limiter`); everything else — output format, title, trim, limiter — comes from the project's `.settings` sidecar, and outputs land beside the source at exactly the paths the GUI would write for the same project — the wav deliverable title-named, the map artifacts named by the source stem like the authoring sidecars.

`warptempo_parser` (`-DWARPTEMPO_BUILD_PARSER=ON`) is parser-only. It reads a project's `.warpmarkers`, `.phaseresetmarkers`, and `.settings` beside the source audio (all three required; the GUI creates them on source load) and writes the map artifacts for the project's `output_format` — the `<source-stem>.warpframemap` plus `<source-stem>.phaseresetframemap` pair for `warptempo_maps`, `<source-stem>.warpframemap` alone for `generic_map`, `<source-stem>.miditempomap` for `midi_map` — without the engine or the GUI. `output_format=wav` is refused: the engine renders wav.

`warptempo_cli` (`-DWARPTEMPO_BUILD_CLI=ON`) is parser plus engine. It reads a full project (the same three required sidecars) and writes the warped wav the GUI would render for it at the same path — `<title>.wav` beside the source, or `limiter=false;<title>.wav` when the settings limiter is off — headless, no display, no GUI toolchain. `output_format` must be `wav`.

`warptempo_engine` (`-DWARPTEMPO_BUILD_ENGINE=ON`) is engine-only and reads no authoring files. It consumes the prebuilt map pair `warptempo_parser` writes beside the source for the same project — the sibling `<source-stem>.warpframemap` and `<source-stem>.phaseresetframemap`, both required (an absent phaseresetframemap refuses the run; an empty one is the valid no-resets form) — and the source wav, runs the PGHI engine directly, and writes `<source-stem>-rendered.wav` beside the source (`limiter=false;<source-stem>-rendered.wav` with `--no-limiter`). The map pair is simply the source's siblings, exactly what `warptempo_parser` writes for the same project, so a parser-then-engine run on a project directory needs no renames. This is the entry point for driving the engine from maps produced by any source, not just this project's authoring files. Phase-reset anticipation is an authoring decision the engine never sees: the parser-side derivation applies it in target/output frames after temporal warping, then inverse-maps the anticipated target frame through the engine map and corrects by `N/2` into the engine's origin-centered source-query domain before the `.phaseresetframemap` is written, so this tool consumes the list as-is; marker files, GUI display positions, and render-view sidecars remain at the authored onset positions.
...
The `output_format` setting is one of `wav` (the default, finished audio), `warptempo_maps` (writes the warp frame map and phase reset frame map pair that drives `warptempo_engine`), `generic_map` (writes the warp frame map alone, for generic external stretch consumers), or `midi_map` (writes the MIDI tempo map for DAW hosts); the map formats write maps rather than audio, and the limiter does not apply to them.
