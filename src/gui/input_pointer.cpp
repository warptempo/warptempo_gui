...
constexpr ToolbarChord kToolbarChords[] = {
    ...
    // the strict-modifier rule, and so is bare `x` (2026-08-18 to 2026-08-24)
    // since 2026-09-13 — it was the VALUE DRAG LAMP's chord from 2026-09-10
    // until that lamp was deleted for a posture the view derives.
    //
    ...
    // IGNORE WAVEFORM MAGNIFICATION (architect 2026-09-17) — bare `]`, a
    // TOGGLE with a lamp, right after `c` and ahead of Follow. Live on a
    // locked tab, under the grid-iterations lock and in the `h` view (a
    // display posture on all three allowlists); it greys nowhere.
...
// TRIM BAR is a disjoint y-band that never answers true
// here (it has its own claim and its own cue). Deliberately NOT the
...
// surface. THE TRIM BAR is a disjoint y-band that never
// answers true here, and neither does the flexible GAP band: that ground is
// chrome, not surface.
//
...
GuiCursorKind GuiInputHandler::pointer_cursor_kind(int x, int y,
    ...
    // the whole span, so the TRIM BRIDGE's cue is the honest one; ON A BOUND
    // it is the matching edge arrow, as the trim endcaps already do. It sits
    // UNDER the modifier arms by their own rank (ctrl there
...
// the trim endcaps already take it, and this is the
// SAME GESTURE ON THE SAME SHAPE — a second member of that family, so it takes
// the family's constant rather than a number of its own.
//
// OVERLAP RESOLVES NEARER-BOUND-WINS WITH TIES TO THE LO BOUND — the trim
// sort's own tie-break over two
...
bool GuiInputHandler::touch_point_in_pan_zone(int x, int y) const {
    ...
    // AND IT YIELDS INSIDE THE OPEN MARKER-LANE EDITOR'S BOX (architect
    // 2026-09-04, on the tablet: "if I tap and drag inside the editor,
    // instead of moving the caret like I would expect on a touch screen, it
    // drags the viewport and the waveform"). The flag box carve-out does NOT
    // reach the editor by itself: the edited marker's flag is not painted
    // while its editor stands and publishes no hit rect (render.cpp's flag
    // pass), so hit_test_flag answers nothing there, the marker lane is nav
    ...
    if (rect_contains(app.flag_editor_box.box, x, y)) return false;
