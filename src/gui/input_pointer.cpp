...
bool GuiInputHandler::dispatch_modal_dialog_button(int index, bool shifted) {
    ...
    // this arm has already been reached under, both acts being unconditional
    // while the panel stands (the report always has rows, so Copy to clipboard
    // never greys; the record is at copy_stats_panel_report). Its row was
    // **Close** alone for its first hours, and that arm read no bit at all.
