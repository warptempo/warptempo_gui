...
bool GuiInputHandler::render_bpm_sweep() {
    ...
    // never accumulates float error and cannot stall: at the 2^53 magnitude
    // an accumulating b += 1.0 rounds back to the same value and loops
    ...
    // sweeps are tens of cells and each cell is a full archival render, so a
    // bracket spanning more than this many whole-bpm steps is refused before
    // anything is allocated. Planner-chosen; retune as needed.
    constexpr double kMaxBpmSweepCells = 10000.0;
    if (cell_count > kMaxBpmSweepCells) {
        // Loud refusal (unlike the silent early-bails above): the user typed
        // a bracket the editor accepted and expects a sweep, so a silent
        // no-op would read as a dead keystroke.
        // Shortest-form print, never an integer cast: the count can exceed
        // the long long range (a bracket spanning 1e308 whole-bpm steps),
        // and casting an unrepresentable double is undefined behavior.
        prompt.open_error_notice(
            "BPM sweep of " + format_value_double(cell_count, 0) +
            " cells exceeds the cap of " +
            format_value_double(kMaxBpmSweepCells, 0) +
            "; narrow the bracket.");
        return false;
    }
