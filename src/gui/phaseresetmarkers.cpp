...
std::string format_phaseresetmarkers_text(
    ...
    for (size_t i = 0; i < markers_.size(); ++i) {
        ...
        // `[#]<frame position>` and nothing else (the grammar at
        // phaseresetmarkers_parse.h). The `#`
        // disable prefix composes ahead of the position, exactly as the parser
        // strips it. No mode suffix — heap is the sole engine, so there is no
        // peak/heap/pass mode to record.
        if (m.disabled) out << '#';
