...
void apply_settings_engine_and_prefs(AppState& app, Viewport& viewport,
    ...
    // NO CAMERA POSTURE OR LAMP LOADS HERE: the hold posture and the follow
    // lamp (AppState::camera_hold / follow) are session state the piece does
    // not determine, and follow left the schema 2026-09-11.
...
std::optional<GuiFailure> source_load_dry_run(
    ...
    if (!presence) return std::move(presence.error());
    if (*presence == SidecarSetPresence::All) {
        ...
        // No sidecar yet: the load stamps the full window on both tabs, which
        // is inside the wall by construction, as the marker templates are.
...
bool GuiFileLoader::load_file(const GuiProjectSource& project) {
    ...
    // THE COMPRESSOR'S TWO NUMBERS ARE THE LIVE DEVICE CONFIG'S
    // (AppState::device_config, gui_main's one struct, read once at startup
    // and never written for these keys): a reopen or a Revert takes the
    // values read then, so a retune of the file is a relaunch.
    const bool ok = next.load(path, app.device_config->waveform_compressor, [&](float) {
    ...
    // refusal below.
    {
        auto presence = sidecar_set_presence(parent, stem);
        if (!presence) {
            // An APPENDED reason is lowercase (notifications.h).
            std::fprintf(stderr,
                "warptempo_gui: Source load aborted: %s\n",
                lowercase_initial(presence.error().diagnostic).c_str());
            gui.request_exit();
            return false;
        }
        if (*presence == SidecarSetPresence::None) {
            create_if_missing(wm_path, "0|1.00\n");
            // The empty file is the canonical blank phase reset sidecar:
            ...
            create_if_missing(set_path,
                              format_default_settings_template(
                                  stem, audio.total_frames()));
        }
    }
    ...
    // no-resets form and parses to an empty list; the required-file rule
    ...
    // switch. The seven wall comparisons live in first_past_eof_wall_defect
