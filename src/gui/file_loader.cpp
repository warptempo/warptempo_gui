...
bool GuiFileLoader::load_file(const GuiProjectSource& project) {
    ...
    // THE GAIN TUNABLES ARE THE LIVE DEVICE CONFIG'S (AppState::device_config,
    // gui_main's one struct, read once at startup): a reopen or a Revert
    // takes the values that were read then, so a retune of the file is a
    // relaunch — nothing re-reads it here.
    const bool ok = next.load(path, app.device_config->waveform_gain, [&](float) {
