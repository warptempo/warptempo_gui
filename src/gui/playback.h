...
// coordinate that buffer frame 0 represents (0 for source.wav, 0 for a
// render-view wav — render view displays the rendered artifact's own timeline,
// so the entry buffer IS its own domain — and the trim-mapped
// full-target-frame anchor for the target buffer) — and
