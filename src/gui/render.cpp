...
void render_waveform(cairo_surface_t* dest,
    ...
    for (int i = 0; i < area.w; i++) {
        ...
        // (raw peaks already rest in range). The upward compression follows
        // inside magnified_tip; being monotone it commutes with the pyramid's
        // min/max, so mapping the reduced extremes is exact. A PICTURE gain:
        // the samples themselves are untouched, here and everywhere.
