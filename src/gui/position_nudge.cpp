...
bool marker_nudge_actionable(const AppState& a, const GuiAudio& audio,
    ...
    // The active column's store through its one selector (app_state.h). The
    // magnification level column never asks in practice — its nudge refuses
    // at active_column_authoring_allowed, ahead of every reader of this —
    // and the selector answers its frame correctly regardless.
