...
// GUI has three display situations — source view, live target view, and
// render view — and the composite rule for which domain is on screen
// (active_audio_view AND render_view.enabled together) lives in exactly
// one place: the accessor is the ONLY reader of app.active_audio_view /
// app.render_view.enabled for DOMAIN QUERIES (position translation,
// domain totals, hit-test positioning). Mode-logic reads — gesture
// gating, view toggles, persistence, playback lifecycle — are outside
// this rule and keep their own flag reads.
