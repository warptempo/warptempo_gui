...
// Naming symmetry: both marker columns share this ONE store
// implementation — GuiWarpMarkers and GuiPhaseResetMarkers are
// GuiMarkerStore instantiated over their per-column GUI marker types, so the
// store mechanics (the sorted-by-time_frame vector, the generation token, and
// the clear-bump-parse-upcast load shape) are identical by construction. The
// per-column vocabulary `warp_X` / `phase_reset_X`, the per-column parse calls
// and serializer contracts, and the GUI-only field docs live at the concrete
...
public:
    ...
    // Monotonically-increasing token bumped on every mutating method — the
    // store's ONE generation, asking only "did anything change". Consumers (the
    // flag-cache fingerprint, the red-flag-set memos)
    // detect any marker-store
    // change by comparing generations rather than diffing contents. There is no
    // coarser row-identity sibling: the STRUCTURAL generation that once stood
    // beside this one existed for the per-tab parked selections, the product's
    // one index held across commands, and went with them (2026-07-29). Nothing
    // holds a store index across commands now, so nothing needs to ask.
