...
// `key=#rrggbb` (no spaces anywhere, lowercase hex, six digits) and terminated
// by a newline. No comments, no blank lines, no trailing content. ANY
// deviation — an unknown key, a key out of order, a duplicate, a malformed or
// uppercase value, a missing final newline, extra bytes — prints ONE stderr
// line naming the file, the 1-based line number and the reason, then REJECTS
// THE WHOLE FILE and keeps every compiled default. Never a partial adoption,
// never an exit: a bad config is a cosmetic problem and the product still runs.
//
