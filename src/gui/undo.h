...
//   (1) REPEAT IDENTITY, for a HELD key: the PHYSICAL press pushes the pre-burst
//       snapshot, and every SYNTHESIZED REPEAT of that press (GuiInputState::
//       synthesized_repeat, set only in GuiPlatform::maybe_fire_repeat) SKIPS its
...
struct Undo {
    ...
    // full shape is at the definition). `synthesized_repeat` is the press's own
    ...
    // REPEAT arm because no command can run between the physical press that pushed
