...
bool read_only_key_blocked(const AppState& app, GuiKey key,
    ...
    // The horizontal arrows step the PLAYHEAD one step in the active column's
    // unit (bare only since 2026-09-23; ctrl held the column for the day
    // before), and
    ...
    // IT ADMITS THE BARE FORM, the one the horizontal pair binds
    // (chord_is_bound, gui_input.h — the ladder retired 2026-09-21, the ctrl
    // form 2026-09-23); an unbound modified arrow needs no admission and is
    // silent whichever side of this gate it lands on.
    ...
    // RESTRICT UNDO TO CURRENT VIEW, bare `z` (2026-09-04): a posture — the
    // bit decides whether an undo RUNS and authors nothing the lock protects, so the switch is admitted on a locked tab and its button
    ...
    // camera the landing owner's (Viewport::land_subject). No ctrl: Ctrl+Tab
    // and the march are their own entries below. No alt: no Alt spelling of
    // Tab binds (the Alt walk was deleted 2026-09-23).
    ...
    // marker's iter bracket (wipe_iter_state).
    ...
    // (1) iteration brackets are NEVER SAVED — session-only, outside the undo
    // domain since 2026-09-10, excluded from the render recipe, so nothing the
    // lock protects reaches disk; (2) the wipe MUTATES NO MARKER PERMANENTLY, clearing
...
// at that point. Anything I want to do, I do before going to iterations mode.
// Iterations mode is just for fine tuning."). The bound cells are the mode's
// only authoring surface; every act that would push an undo entry refuses, and
// undo and redo refuse with it, because the mode's own surface pushes nothing
// and an entry made under a lit lamp could not be told from one made before it.
//
...
// lamp cannot be locked — bare `o` is delta (a)'s newest member, below — and
// the lamp cannot be lit while ANY tab is locked, bare `i`'s own arm asking
// any_tab_read_only (app_state.h) beyond what the wider list has ever said
// about it. The
...
//     keys need no entry here: an open editor claims the keyboard above this
//     gate (keyboard_modal_editor_active).
//   * CTRL+Z AND CTRL+SHIFT+Z — admitted NOT TO ACT but so that their own
//     dispatch arm can raise the sentence that names undo
//     (kIterationLockUndoCard / kIterationLockRedoCard, app_state.h). This
//     gate's card names the way out of the mode, which is the right answer for
...
bool iteration_lock_key_blocked(const AppState& app, GuiKey key,
    ...
    const bool bound_axis = (app.addressed_cell == MarkerCell::Lower ||
                             app.addressed_cell == MarkerCell::Upper);
    if (bound_axis) {
        // Up / Down at all three magnitudes — bare, shift and ctrl — which is
        // `!alt && !(ctrl && shift)`, the dispatch arm's own test.
        if ((key == GuiKeys::Up || key == GuiKeys::Down) &&
            !alt && !(ctrl && shift))
            return false;
        // Return / KpEnter, plain only, the editor-open arm's own test.
...
//     IsoLeftTab exactly as the live cycle is, its landing's camera the
//     landing owner's exactly as the live walk's is (Viewport::land_subject);
...
bool history_mode_owns_key(GuiKey key, GuiInputState mods) {
    // EVERY ALT SHAPE IS REFUSED: the mode claims no alt chord (the
    ...
    // 2026-09-26, each step centred by `c`, live and here alike) — so the
    // chord means one thing everywhere and only
...
// mode's Ctrl+Shift+Tab march composes it too (2026-08-18). Two callers, both
...
void GuiInputHandler::cycle_history_diff_flag_focus(bool forward,
    ...
    // the march runs the mode's `c` (run_center_command's mode arm) behind
    // it, which writes the working zoom and centres again.
    switch (frame) {
...
bool GuiInputHandler::handle_history_mode_key(GuiKey key, GuiInputState mods) {
    ...
    if (key == GuiKeys::H) {
        ...
        // the lamp stands (iteration_lock_greys, app_state.h), and the grey is
        // all it says — the sentence is this press's, the refusal-reason
        // tooltip class having gone on 2026-09-12. The view's OWN buttons — Load in place, Revert and
    ...
    if (mods.ctrl && mods.shift && key == GuiKeys::Tab) {
        // THE PAIRED MARCH, MODE-LOCAL — THE LIVE MARCH'S ROUND TRIP over the
        // view's own vocabulary (architect 2026-09-26; the chord first came
        // in here 2026-08-18): switch to the other tab, step, centre with the
        // mode's `c`; switch home, centre with the mode's `c`. The user ends
        // on the tab he started from and never sees the other one — the whole
        // act is one key dispatch and nothing in it commits a frame (the rule
        // and the saved-band mechanics are at the live march,
        // handle_tab_switch_keys).
        //
        // THE CENTRING IS THE MODE'S `c` EXACTLY (run_center_key_command, whose
        // mode arm in run_center_command re-lands the playhead on the focused
        // diff flag, writes the working zoom and centres, and whose tail arms
        // the hold posture) — the live march's own call, the fork being
        // inside it.
        //
        // ONE STEP, NOT TWO, and that is the honest re-expression of "advance
        // one marker next in both tabs": the mode's focus is ONE index over
        // one diff-flag list, shared by both tabs (the tabs share both marker
        // stores, so the list is the same on either side, and the tab switch
        // leaves the index standing). The other tab's step moves that shared
        // focus to the next flag; a second step at home would move it again
        // and put the visible tab two flags on, past the one the other tab
        // stands on. So the home half is the mode's `c` alone, which lands
        // the home tab's playhead on the same focused flag — both tabs end on
        // one flag, which is lockstep. A step that refuses at the last flag
        // (the cycle's silent wall) leaves the focus where it stood, and both
        // `c`s centre on it, as a bare `c` would.
        //
        // No preview trigger, the live march's reason (the round trip ends on
        // the tab it started from); the view has no playback in any case.
        const char home = app.active_tab_view;
        active_views.switch_active_tab_view_to(home == 'A' ? 'B' : 'A');
        cycle_history_diff_flag_focus(true, MarkerLandingFrame::Center);
        run_center_key_command();
        active_views.switch_active_tab_view_to(home);
        run_center_key_command();
        return true;
    }
...
// keyboard, or typing `c` into a flag would recentre the camera. So this
...
// family's, not this ring's, and alt binds nothing in it, as it binds no Tab
// anywhere.
...
bool GuiInputHandler::repeat_eligible(GuiKey key, GuiInputState mods) const {
    ...
    // 2026-08-31, and more plainly so since 2026-09-04): a plain press is a
    // whole FILE — the next track on End, and on Home the item's
    ...
    // first line exactly as every ring's does. THE
    ...
    // is the continuous step this eligibility is for. NOTHING HERE ANSWERS FOR
    // THE CAR: a head unit's button is a direct act on the player and never
    // enters this road at all.
    ...
    // (the horizontal ladder retired 2026-09-21; the Ctrl form 2026-09-23),
    // so a held Left / Right — key or button — walks one step of the active
    // column's unit a fire, a hop on the phase-reset column, keeping its
    // column at every fire while the hold posture stands. The lane
    // split and the unit are decided per fire at dispatch, so the arm is
    // their own rather than a term in the bare list above; CTRL, SHIFT and
    // ALT spell nothing to repeat.
    ...
    // (mirroring the dispatch arm), all requiring no ctrl and no alt (no Alt
    // spelling of Tab binds, so none repeats). The `h` history
    ...
    if (mods.ctrl && !mods.alt && key == GuiKeys::Z)
        return true;
    // `=` / `-` AND CTRL+0 ARE DELIBERATELY ABSENT because they bind
    // nothing at all (the stepped zoom was removed 2026-09-25: zoom is on
    // every surface) — and an unbound chord has no repeat to ask about.
    return false;
}
...
void GuiInputHandler::run_iteration_sweep_render() {
    ...
    // configuration, and a SWEPT reset's time_frame is the only thing a cell
    // moves in it.
    const std::vector<GuiPhaseResetMarker> base_phase_resets =
        app.phaseresetmarkers.markers();
    // THE INVERTED-BRACKET BREACH, ONE COMPOSER FOR BOTH COLUMNS (architect
    // 2026-08-30 for its two surfaces; both columns since 2026-09-09). The
    // editors enforce start <= end and both brackets are session-only, so
    // start > end is an internal breach — refuse the dispatch loudly and
    // enqueue nothing, repairing no iter state (the state is evidence; the
    // bracket lifecycle owns wiping). The stderr line keeps the COLUMN and the
    ...
    for (int i = 0; i < warp_axis_n; ++i) {
        ...
        if (swept) {
            ...
            if (start_cents > end_cents) {
                refuse_inverted_bracket("warp marker", i);
                return;
            }
    ...
    // fire, and exiting the mode IS the bracket clear on BOTH COLUMNS
    // (wipe_iter_state, the chokepoint every other iter-mode exit runs). Safe
    // here: every request above carries its own per-cell marker AND
    // phase-reset copies, so nothing dispatched reads the live iter fields.
    //
    ...
    // argument, and narrower: iter brackets are SESSION-ONLY fields on both
    // columns — never serialized (no sidecar key, and the two slices drop them
    // at the render boundary), excluded from the render recipe, and outside
    // the undo domain (2026-09-10, so the wipe pushes nothing at all) — so the
    // write can reach neither disk nor a render nor a history entry, and the
...
void GuiInputHandler::apply_recipe_in_place(
    ...
    // the two sweep-mode bits are VIEW STATE, exactly like the S/T and W/P
    // bits this act already leaves alone, and the mode bit stays as the user
    // set it — dark, on every road that reaches here. Until that date the
    // block reset every bracket, wiped the bpm scratch and turned both bits
    // off inline, a fourth exit route beside the three that run the one
    // chokepoint wipe_iter_state (flag_editor.cpp).
    //
    ...
    // snapshot, whose session-only bracket fields are EMPTY by construction
    // since 2026-09-10, every push stripping them (strip_iter_fields). So no
    // road can carry a bracket in, and the lock means none can be standing to
    // be carried out.
    //
...
void GuiInputHandler::open_project_picker() {
    ...
    // first", 2026-08-30 to 2026-09-02, and the CLOSE-THEN-OPEN body that
    // replaced it for the one day the File anchor stayed lit under the panel.
    // The two modes still never stand together; what keeps that true now is
    // that neither road exists.)
    // A STANDING PICKER IS SILENT: the picker IS this act, so a second
    ...
    // row, which the panel makes unreachable in the same way — so this arm is
...
bool GuiInputHandler::handle_mode_keys(GuiKey key, GuiInputState mods) {
    ...
    if (key == GuiKeys::I && !ctrl && !shift && !alt) {
        ...
        // per-tab by nature. So the gate answers a locked ACTIVE tab and this
        // arm answers a locked OTHER one, IN THE SAME WORDS: one press of `i`
        // says one sentence whichever tab is the locked one, which is why the
        // English has one owner (read_only_chord_card, notifications.h) and
        // this is a second caller of it rather than a second spelling.
        //
        // IT NEEDS NO LAMP TERM, and the off edge is untouched by it: this is
        // the exclusion's other half — no tab can be LOCKED while the lamp is
        // lit, bare `o` and the settings editor's two typed self-lock keys
        // both refusing — so under a lit lamp the predicate is false by
        // construction and the toggle-off below is always reachable. The
        // Grid Iterations button greys on this same answer
        // (redesign_button_enabled's IconIter arm, app_state.h), so the face
        // and the key refuse together.
        if (any_tab_read_only(app)) {
            notifications.notify(
                AppState::NotificationClass::Normal,
        ...
        if (app.iteration_mode_enabled) {
            ...
            // run_iteration_sweep_render's tail where the class is argued in
            // full): iter brackets are session-only fields, never serialized
            // and excluded from the render recipe, so the write reaches
        ...
        // selection has been handed over to the act — the same use-case edge
        // every marker verb takes, said through the one writer
...
bool GuiInputHandler::route_render_player_key(GuiKey key, GuiInputState mods) {
    ...
    // pair because each press is a whole file.
    //
    ...
    switch (key) {
        ...
        case GuiKeys::Up:
            ...
            // Down's: the arrow is the fast walk through a listing and must
            // never leave one by itself. THE UP-A-FOLDER EXIT IS THE UP
            // BUTTON'S, BACKSPACE'S AND THE CAR'S PREVIOUS, and no arrow's and
            // no skip's. (This arm carried the left skip's row-0 rule for the
            // hours between his two rulings of that day.)
            render_player.move_highlight(-1);
            return true;
        case GuiKeys::Down:
            // Bare Down is the walk and nothing else — the listing's last row
            // is its silent wall, exactly as bare Up's is row 0.
            render_player.move_highlight(+1);
            return true;
        case GuiKeys::Left:
        ...
        case GuiKeys::Right:
            ...
            return true;
        case GuiKeys::Home:
            // THE LEFT SKIP'S ACT — this key and the Home button's plain press,
            // TWO ROADS AND NO MORE (architect 2026-09-12, from the car: "the
            // car is a separate interface", so the head unit's own Previous is
            // its own act and reaches nothing here). Two arms over one position
            // test: THE PREVIOUS ENTRY of the item's folder inside the item's
            // first three seconds (the previous-track window at
            // kPlayerPreviousThresholdMs, asked in every transport state), and
            // everywhere else this track's own start. IT NEVER LEAVES A
            // FOLDER — that is the Up button, Backspace and the car's Previous
            // — the key having gone up for the one day on which the skips
            // walked the band. One-shot: each press is a whole FILE, so a hold
            // could only walk the folder a file per repeat
            // (repeat_eligible's player arm).
            render_player.home();
            return true;
        case GuiKeys::End:
            // THE RIGHT SKIP'S ACT — this key and the NextTrack button's plain
            // press, the same two roads (the car's Next is the car's own act).
            // IT IS THE NEXT SONG in every transport state (architect
            // 2026-09-04, from the
            ...
            // lamp says, live or paused or idle alike; at the folder's last
            // wav it is a silent walled no-op, nothing looping, and the button
            // greys on that same wall.
            // IT WAS THE TRACK'S OWN END from 2026-08-30 to
            // 2026-09-04 — a seek
...
void GuiInputHandler::jump_to_value_source() {
    ...
    run_center_command();
    // Its closing `c` is an explicit centring, so the jump arms the HOLD
    // posture behind it (AppState::camera_hold).
...
bool GuiInputHandler::handle_tab_switch_keys(GuiKey key, GuiInputState mods) {
    ...
    // into and the refusal, its per-tab owner and the two tab buttons' greys
    // all went with it.)
    ...
    // Ctrl+Shift+Tab — THE PAIRED MARCH, A ROUND TRIP (architect 2026-09-26):
    // advance both tabs' marker focus one step in lockstep and END ON THE TAB
    // IT STARTED FROM. "If I'm in tab B and I hit Ctrl+Shift+Tab, I should
    // advance one marker next in both tabs and then end up in tab B. So
    // basically I should not see tab A at all." Showing the other tab is the
    // A/B audition's business (Shift+Space, GuiAbAudition), which switches to
    // it to play a portion and back; the march walks the pair in lockstep,
    // and alternating the tab in view was only ever a consequence of the
    // audition not existing yet.
    //
    // THE ORDER: switch to the OTHER tab, step its focus one marker forward
    // and centre it as bare `c` does; switch HOME, step its focus and centre
    // it the same way. The visible tab's own step is the last camera write.
    // Each tab steps from its OWN focus: the switch clears the selection and
    // the coincidence auto-select re-seats whatever marker stands under that
    // tab's parked cursor, so two tabs resting on one marker both advance to
    // the next.
    //
    // EACH STEP CENTRES THROUGH `c`'s OWN ACT (run_center_key_command): the
    // working zoom, centred on the focused marker, the playhead on it, the
    // hold posture armed. THE MARCH WRITES THE ZOOM — the working zoom, on
    // both tabs — where the bare Tab walk (below) still writes none and lands
    // through the landing owner's walk. The step states
    // MarkerLandingFrame::Center, `c`'s own statement to the jump, so the
    // step itself frames nothing `c` does not immediately re-frame. A step
    // that refuses at a wall (cycle_marker_focus's silent refusal) leaves that
    // tab's focus where it stood, and its `c` still centres on it, as a bare
    // `c` would.
    //
    // THE USER NEVER SEES THE OTHER TAB: the whole act runs inside this one
    // key dispatch, and nothing in it commits a frame — each switch's and
    // each zoom's synchronous rebuild (kick_waveform_sync) renders into the
    // off-screen plate cache and posts damage, and the compositor gets one
    // paint after the dispatch returns, of the home tab. The other tab's
    // centred band persists: switch_active_tab_view_to pushes the live
    // viewport / zoom / playhead into the LEAVING tab's band
    // (refresh_active_tab_view_from_app) before pulling the entering one's, so
    // the switch home saves the other tab's landing, and that tab restores it
    // at the next Ctrl+Tab. Do not collapse the two steps into one.
    //
    // NO PREVIEW TRIGGER: Ctrl+Tab's target_render.trigger() re-dispatches the
    // preview for the ENTERING tab's trim, and the round trip enters the tab
    // whose trim the preview is already bound to, the audition's own reason
    // for switching without it (ab_audition.h). The playback stop is the
    // switch's own (switch_active_tab_view_to).
    //
    ...
    // the act rather than keep the workaround that had it repeat the first
    ...
    // writes no camera).
    //
    // The `h` view's own Tab arm states the same Land over its diff-flag
    // cycle (handle_history_mode_key). The Ctrl+Tab branch above
    // runs first and returns, so Ctrl+Tab is consumed before reaching here;
    // the explicit !ctrl guards below keep Ctrl+Shift+Tab out of the cycle
    // path, and the !alt guards make every Alt spelling of Tab an unbound
    // no-op (the Alt walk, 2026-09-22, was deleted 2026-09-23).
    constexpr MarkerLandingFrame walk_frame = MarkerLandingFrame::Land;
...
void GuiInputHandler::run_waveform_lane_playhead_step(
    ...
    // step the press takes (horizontal_arrow_step_actionable).
    // THE CAMERA, AT EVERY ZOOM, IS THE HOLD POSTURE'S (architect 2026-09-23;
    // the press's modifier chose it from 2026-09-22): dark, the step follows
    // the edge — the landing's own keep-visible edge-align and nothing more,
...
void GuiInputHandler::handle_plain_bare_keys(GuiKey key) {
    ...
    case GuiKeys::Escape:
        ...
        break;
    case GuiKeys::Left:
        // THE WAVEFORM-LANE STEP — one step back in the active column's unit
        // (horizontal_arrow_step, gui_input.h: a painted column, or a hop on
        // the phase-reset column), through the act owner directly above this
        // dispatch, whose camera is the hold posture's; the whole body lived
        // here until 2026-08-31.
