...
// Open the PhaseResetMode editor on phase_reset_markers[idx]. Mirrors
// enter_top_flag_edit's reseat (minus the click-x cursor placement and the
// warp selection/playhead update — phase-reset entry is mode-only). The
// editable payload is the mode token; the timestamp (+ '#' when disabled)
// and the pipe ride the locked prefix, exactly as save() writes them.
void GuiFlagEditor::enter_phase_reset_mode_edit(int idx) {
    if (app.active_markers_view != 'P') return;
    const auto& prv = app.phase_reset_markers.markers();
    if (idx < 0 || idx >= static_cast<int>(prv.size())) return;
    const GuiPhaseResetMarker& m = prv[idx];

    std::string locked_prefix;
    if (m.disabled) locked_prefix += '#';
    locked_prefix += format_timestamp(m.time_seconds);
    locked_prefix += '|';

    // Discard any prior edit on a different target before reseating the
    // shared editor (mirror enter_text_edit).
    if (text_editor::is_active(app.top_flag_editor) &&
        app.top_flag_editor.target != idx) {
        text_editor::deactivate(app.top_flag_editor);
    }
...
// Validate the PhaseResetMode editor's pending token and, on success, write
// the parsed mode onto phase_reset_markers[idx] under the phase-reset undo
// op (the same op delete/nudge use), then re-render so the heard output
// updates. On parse failure: red-flash, leave the editor open, commit
// nothing.
void GuiFlagEditor::commit_phase_reset_mode_edit() {
    if (!text_editor::is_active(app.top_flag_editor)) return;
    const int idx = app.top_flag_editor.target;
    const auto& prv = app.phase_reset_markers.markers();
    if (idx < 0 || idx >= static_cast<int>(prv.size())) {
        // Target became invalid (e.g. file reload). Drop the edit.
        this->exit_top_flag_edit_no_commit();
        return;
    }

    Mode parsed_mode;
    if (!mode_from_token(app.top_flag_editor.pending, parsed_mode)) {
        app.top_flag_editor.red = true;
        viewport.invalidate_top_strip();
        std::fprintf(stderr,
            "warptempo_gui: phase-reset mode edit rejected: "
            "expected peak/heap/pass: %s\n",
            app.top_flag_editor.pending.c_str());
        return;
    }

    // Snapshot pre-edit phase-reset state per the existing phase-reset undo
    // op before mutating.
