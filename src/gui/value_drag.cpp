...
bool ValueDragOps::begin(int marker, MarkerCell cell, int press_y) {
    ...
    if (cell == MarkerCell::Payload) {
        // THE TEMPO ARM, SEEDED BY THE OWNER'S OWN AUTHORED CENTS: the target
        // rule admits a tempo owner alone (a pass is refused by target view's
        // own kind gate), so the stored field IS the value the flag shows.
...
void ValueDragOps::commit() {
    ...
    // CENTS are the whole compare: the motion arm writes that field and no
    // other.
    const std::vector<GuiWarpMarker>& mv = app.warpmarkers.markers();
    ...
    if (now.tempo_cents == was.tempo_cents) return;
