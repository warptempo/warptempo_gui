...
bool ValueDragOps::begin(int marker, MarkerCell cell, int press_y) {
    ...
    // a pointer gesture's non-event is its own answer, and where the view makes
    // this the flag's plain drag (value_drag_posture) a flag that cannot be
    ...
    // for every cell on the magnification level column (the plain flag drag
    // there is the HORIZONTAL move, value_drag_posture's own 'M' arm), so the
    // `== 'P'` forks below and in the motion body never meet an M drag.
    ...
    if (cell == MarkerCell::Payload) {
        ...
        // and then applies on up and down"; restored 2026-09-13 when the
        // arrow step began converting a pass in target view too). The seed is
        // one body for the two hands, which is what keeps the pointer and the
        // keyboard from resolving the same pass differently.
        ...
        // content, so the snapshot is captured here for the commit's one entry.
        const std::vector<GuiWarpMarker>& mv = app.warpmarkers.markers();
        if (marker >= static_cast<int>(mv.size())) return false;
        st.start_value =
...
void ValueDragOps::apply_motion(int mouse_y) {
    ...
    if (app.value_drag.cell == MarkerCell::Payload) {
        ...
        // both directions and why the step loop's skip carries the same
    ...
    } else if (app.value_drag.cell == MarkerCell::Measure) {
        // THE MEASURE WRITE, through the arrows' landing owner as a delta from
        // the value the store holds right now: a blank counts as 0 and every
        // landing clamps into [1, 999] silently. The spelling is the
        // grammar's own writer. Asked before marker_mut for the tempo arm's
        // reason (no generation bump on a landing that stands still).
        const std::vector<GuiWarpMarker>& mv = app.warpmarkers.markers();
        if (idx < 0 || idx >= static_cast<int>(mv.size())) return;
        const std::string& now = mv[static_cast<size_t>(idx)].measure;
        const int64_t cur = measure_step_start(now);
        MarkerMeasureValue landed;
        landed.whole = measure_step_landing(cur, target - cur);
...
void ValueDragOps::commit() {
    ...
    // NO TEMPO TAIL: the field is not a map input, so there is no re-warp, no
