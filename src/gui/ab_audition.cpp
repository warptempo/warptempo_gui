...
void GuiAbAudition::apply_working_zoom() {
    ...
    // The Center on next marker lamp takes no special case: `c`'s zoom is a
    // commit like any `c`, and writes the lamp only if it crosses the working
    // level (commit_center_on_next_marker_zoom, app_state.h).
...
bool GuiAbAudition::launch_phase(GuiAuditionSequence::Phase phase,
    ...
    // clear having wiped whatever stood): the launch body finds the act
    // standing on this phase, and the body clears no sequence on this road
    ...
    if (!playback_lifecycle.launch_bounded_audition(start,
        ...
        // where it is, the interrupt rule's own answer, through the one
        // spelling of "the act is over".
        clear_audition_sequence(app);
        return false;
    }
    return true;
