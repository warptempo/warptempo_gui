...
//
// AUTHORED domain (format_frame_double / parse_frame_double): authored
// positions — marker times on both columns and both trim bounds — are whole
// source frames (~23 microseconds each at 44.1 kHz), serialized as plain
// integer text. These two helpers are the single definition of the on-disk
// authored position form; every producer and consumer of an authored
// position — the marker parsers, the settings trim reader, the serializers,
// and the CLI — goes through exactly this pair, so persistence round-trips
// bit-exactly and there is no second representation to disagree with the
// walls. Fractional, negative, non-finite, or otherwise malformed position
// text is a state the GUI can never write — adversarial, load-fatal at every
// consumer. In-memory storage is currently integer-valued doubles (a
// follow-up retypes to int64_t), so every exact-compare wall and validator
// is unchanged. Timestamps (MM:SS.mmm) are display-only renderings; see
// time_format.h.
//
...
// Authored frame double -> plain integer text ("0", "44100"). The input is
// integer-valued by contract (authored positions are whole source frames),
// so the cast is exact; routing through the integer to_chars also pins the
// spelling to plain digits — no scientific form, so 600000 prints as
// "600000", never "6e+05".
...
// Text -> authored frame double. Strict: the whole field must be consumed,
// and the value must be finite, non-negative, and a whole frame. Rejecting
// fractional values alongside NaN, inf, negative, empty, and trailing junk
// is the parser's malformed-position rule — adversarial, load-fatal, the
// same category as a malformed timestamp used to be. The restriction is on
// the VALUE, not the spelling: integer-valued forms such as "44100.0" or
// the historical scientific "6e+05" still parse (and re-serialize as plain
// integer text). Returns true and sets `out` on success; returns false and
