...
// warp-marker time->frame convention in build_warp_maps. Refuses an enabled reset
// authored past the source end (strictly greater than total_frames; equal is
// allowed), the producer-side validation layer parallel to build_warp_maps'
...
// before the check — only resolved markers are validated, as in build_warp_maps —
