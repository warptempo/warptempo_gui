...
// total rule, shared by the GUI target-view cache and the render-entry
// `.settings` write clamp, so the rounding and fallback can never drift
// between the consumers. An empty map returns the source total
