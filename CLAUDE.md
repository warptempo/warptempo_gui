...
Position gestures anchor to the painted column grid through `authored_frame_at_column` → `snap_authored_frame`; walls win over the grid. Waveform width floors to a multiple of 16 px; `clamp_viewport_start` is the one viewport chokepoint, zoom clamp included. Zoom is continuous in [1, 17] at `kZoomBaseMsPerPx × 2^(level−1)` ms/px and rests where a gesture ends (`clamp_zoom_level`). `c` = working zoom (`kWorkingZoomLevel`); `=` / `-` and Zoom In / Out step one level about the viewport's centre (`Viewport::apply_zoom_step`), greying at their walls (`zoom_in_step_actionable` / `zoom_out_step_actionable`); `0` = full zoom out, stamping `zoom_recall_level` and setting `whole_song_visible` (kept across S/T flip and resize), a second `0` running `c` at the stamped level. `Shift+0` is Reset Trim, not a zoom.

No camera behaviour is derived from the zoom level. Bare Tab (live and `h`) centres its landing at the standing zoom (`MarkerLandingFrame::Center`); Alt+Tab / Alt+Shift+Tab is the same walk with the least-movement landing (`Viewport::least_movement_scroll_if_needed`: onscreen moves nothing, offscreen lands the edge margin in). Bare Left/Right follow the edge; Ctrl+Left/Right hold the subject's column (`Viewport::hold_subject_column_after_nudge`, `NudgeCamera`), never centring. Only `c`, `0`'s second press, bare Tab, the tab-comparing acts, the paired march, Shift+J and the A/B audition centre; drags, clicks, pans, undo and Ctrl+Tab leave the camera.
...
Follow is a one-shot: `follow_armed` (bare `f`) is spent by the next project-audio launch (`spend_follow_lamp`), the play chasing on `follow_engaged`, which any pan, the placement click or the stop clears; the audition and render player ignore it and a refused launch spends nothing; `toggle_follow` is the one fork, the face showing the chase during play.
...
- Alt binds only the seven Ctrl+Alt chords (the two renders, Ctrl+Alt+P / Ctrl+Alt+Shift+P / Ctrl+Alt+M, Ctrl+Alt+O, Ctrl+Alt+Shift+M) and Alt+(Shift+)Tab. Alt+Tab is admitted wherever bare Tab is, per `chord_is_bound`.
- Bare `\` is Synchronize.
...
Every chrome button acts at the lift (`AppState::ChromePress`), except the menu anchors, which act at the press. The hold-repeating buttons are the arrows, the two hold-column nudges, and Undo/Redo (`kToolbarChords`' `repeats`). A held repeat outranks the long-press shift. A wall no-op runs before the coalesce stamp, and a held repeat on an empty stack is silent. Every hold threshold reads `kHoldBeatMs` = 575, whose readers are inventoried at gui_input.h. Wayland's key delay comes from `repeat_info` and is never hard-coded. Every shift-capable gesture whose bare form has a button admits that button's shift-click and long press, per `redesign_button_shift_admits`.

The arrow ladder is vertical only: Up/Down steps are 1 bare, 3 with Ctrl and 10 with Shift (`arrow_step_magnitude`). Left/Right take no rung, and Shift+Left/Right are silent. Ctrl+Left/Right holds the subject's column. The bare horizontal unit is one column on W/M and one hop on P (`horizontal_arrow_step`).
